...
func insertCabinet(db *sql.DB, cab *Cabinet) error {
	return db.QueryRow(`
		...
		orDefault(cab.Status, "unknown"),
		orDefault(cab.DoorStatus, "unknown"),
		orDefault(cab.PowerStatus, "normal"),
		cab.Notes,
	).Scan(&cab.ID, &cab.CreatedAt, &cab.UpdatedAt)
}

func listCabinets(db *sql.DB) ([]Cabinet, error) {
	rows, err := db.Query(`
...
func getCabinetByID(db *sql.DB, id int) (*Cabinet, error) {
	row := db.QueryRow(`
		SELECT id, reference, name, zone, address, latitude, longitude,
			status, door_status, power_status,
			voltage_l1, voltage_l2, voltage_l3,
...
func listCabinetCircuits(db *sql.DB, cabinetID int) ([]CabinetCircuit, error) {
	rows, err := db.Query(`
...
func insertCabinetCircuit(db *sql.DB, cc *CabinetCircuit) error {
	return db.QueryRow(`
		INSERT INTO cabinet_circuits (cabinet_id, name, phase, circuit_number, status, lamp_count)
		VALUES ($1,$2,$3,$4,$5,$6)
		RETURNING id, created_at, updated_at`,
		cc.CabinetID, cc.Name,
		orDefault(cc.Phase, "L1"),
		cc.CircuitNumber,
		orDefault(cc.Status, "active"),
		cc.LampCount,
	).Scan(&cc.ID, &cc.CreatedAt, &cc.UpdatedAt)
}

func scanCabinetRow(scan func(...any) error) (Cabinet, error) {
	var cab Cabinet
