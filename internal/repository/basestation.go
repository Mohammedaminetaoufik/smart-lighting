...
// InsertBasestation inserts a new basestation.
func InsertBasestation(db *sql.DB, b *models.Basestation) error {
	return db.QueryRow(`
		...
		utils.OrDefault(b.Status, "unknown"),
		utils.OrDefault(b.NetworkType, "Simulated"),
		utils.OrDefault(b.PrimaryBackhaul, "simulated"),
		b.ActiveBackhaul,
		b.ConnectedNodesCount, b.DisconnectedNodesCount,
		b.SignalQualityAvg,
		utils.OrDefault(b.BatteryStatus, "unknown"),
...
func scanBasestationRow(scan func(...any) error) (models.Basestation, error) {
...
// ListBasestations returns all basestations.
func ListBasestations(db *sql.DB) ([]models.Basestation, error) {
	rows, err := db.Query(basestationSelectCols + ` ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var list []models.Basestation
	for rows.Next() {
...
// GetBasestationByID fetches a single basestation by ID.
func GetBasestationByID(db *sql.DB, id int) (*models.Basestation, error) {
...
// UpdateBasestationStatus updates the status and last_seen_at of a basestation.
